...
{ /* char is sorted at left and padding at buffer's right */
	int i = 0;
	char padd = ' ';
	...
	int length, char padd, char extra_c)
{
	...
	if (width > length)
	{
		for (i = 1; i < width - length + 1; i++)
			buffer[i] = padd;
		buffer[i] = '\0';
		if (flags & F_MINUS && padd == ' ')/* Assigh extra char to left of buffer */
		{
			if (extra_c)
				buffer[--ind] = extra_c;
	...
	int width, int flags, char padd, char extra_c, int padd_start)
{
	...
	if (width > length)
	{
		...
		else if (!(flags & F_MINUS) && padd == ' ')/* extra char to left the buffer*/
		{
			buffer[--ind] = 'x';
			buffer[--ind] = '0';
			if (extra_c)
